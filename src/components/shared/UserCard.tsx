...
const UserCard = ({ user }: UserCardProps) => {
  ...
  return (
    <Link 
      ...
      className="group relative overflow-hidden bg-gradient-to-b from-dark-3 to-dark-2 border border-dark-4 rounded-2xl p-6 transition-all duration-300 hover:border-primary-500/30 hover:shadow-lg hover:shadow-primary-500/5 hover:-translate-y-1"
    >
      ...
      <div className="relative flex flex-col items-center gap-4">
        ...
        <div className="relative">
          ...
          <img
            ...
            alt={user.name}
            className="relative w-16 h-16 rounded-full object-cover ring-2 ring-dark-4 group-hover:ring-primary-500/50 transition-all duration-300"
          />
          {/* Online indicator placeholder */}
          <div className="absolute bottom-0 right-0 w-4 h-4 bg-green-500 rounded-full border-2 border-dark-2" />
        </div>

        {/* User Info */}
        <div className="flex flex-col items-center gap-1 text-center">
          <p className="font-semibold text-light-1 line-clamp-1 group-hover:text-white transition-colors">
            {user.name}
          </p>
          <p className="text-sm text-light-3 line-clamp-1">
            @{user.username}
          </p>
        </div>
        ...
        {!isOwnProfile && (
          <Button 
            type="button" 
            size="sm" 
            className={`w-full mt-1 rounded-xl font-medium transition-all duration-300 ${
