...
const UserCard = ({ user }: UserCardProps) => {
  ...
  return (
    <Link 
      ...
      className="block bg-dark-2 border border-dark-4 rounded-xl md:rounded-2xl p-4 md:p-6 transition-all duration-300 hover:border-primary-500/30 hover:shadow-lg"
    >
      <div className="flex flex-col items-center gap-3 md:gap-4">
        ...
        <div className="relative">
          <img
            ...
            alt={user.name}
            className="w-12 h-12 md:w-16 md:h-16 rounded-full object-cover ring-2 ring-dark-4"
          />
          {/* Online indicator */}
        ...
        <div className="flex flex-col items-center gap-0.5 md:gap-1 text-center w-full">
          <p className="font-semibold text-light-1 text-sm md:text-base line-clamp-1">
            {user.name}
          </p>
        ...
        {!isOwnProfile && (
          <Button 
            type="button" 
            size="sm" 
            className={`w-full mt-1 rounded-lg md:rounded-xl font-medium text-xs md:text-sm py-2 ${
              isCurrentlyFollowing 
                ? "bg-dark-4 hover:bg-red-500/20 hover:text-red-400 text-light-2" 
