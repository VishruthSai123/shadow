...
export class NotificationSound {
  ...
  private isSoundEnabled(): boolean {
    ...
    const enabled = localStorage.getItem('shadow_sound_notifications');
  ...
  private getVolumeLevel(): number {
    ...
    const volume = localStorage.getItem('shadow_notification_volume');
    return volume ? parseFloat(volume) : 0.1; // Default to 10% volume
  }

  setSoundEnabled(enabled: boolean) {
    if (typeof window !== 'undefined') {
      localStorage.setItem('shadow_sound_notifications', enabled.toString());
    }
  }

  setVolumeLevel(volume: number) {
    if (typeof window !== 'undefined') {
      localStorage.setItem('shadow_notification_volume', Math.max(0, Math.min(1, volume)).toString());
    }
  }
